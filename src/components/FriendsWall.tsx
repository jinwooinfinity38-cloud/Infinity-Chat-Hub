...
import { X, Plus, Image as ImageIcon, Send, Settings, User, Heart, MessageSquare, Paperclip, Smile } from 'lucide-react';
import { User as FirebaseUser } from 'firebase/auth';

interface FriendsWallProps {
  isOpen: boolean;
  onClose: () => void;
...
export function FriendsWall({ isOpen, onClose, user, userData }: FriendsWallProps) {
      ...
      <Dialog open={isOpen} onOpenChange={onClose}>
        <DialogContent className="bg-[#121212] border-none text-white max-w-4xl w-[95vw] h-[90vh] p-0 rounded-2xl overflow-hidden flex flex-col shadow-2xl">
          ...
          <div className="p-4 border-b border-zinc-800/50 flex items-center justify-between">
            <h2 className="text-lg font-extrabold">Friends wall</h2>
            ...
            <ScrollArea className="flex-1 px-4">
              {posts.length === 0 ? (
                <div className="h-full flex flex-col items-center justify-center py-20 text-center">
                  ...
                  <p className="text-zinc-500 font-medium text-lg">There are no posts to show</p>
                </div>
              ) : (
        ...
        <DialogContent className="bg-[#1c1c1c] border-none text-white max-w-md w-[90vw] p-0 rounded-3xl overflow-hidden shadow-2xl">
          <div className="p-6">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-extrabold tracking-tight">Create a new post</h2>
              <Button 
                variant="ghost" 
                size="icon" 
